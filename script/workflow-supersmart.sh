...
if [ ! -e "$WORKDIR/$BACKBONE" ]; then
    smrt bbinfer -i examl -b 100 -s $WORKDIR/supermatrix.phy -m -o $BACKBONE -w $WORKDIR -l bbinfer.log
fi
...
if [ ! -e "$WORKDIR/consensus.nex" ]; then
    smrt consense -i $WORKDIR/chronogram.dnd -w $WORKDIR
fi

smrt bbdecompose -b $WORKDIR/consensus.nex -a $WORKDIR/aligned-smrt-inserted.txt -t $WORKDIR/taxa-replicated.tsv -w $WORKDIR
