...
final <- read.tree('../results/2016-02-04-clusters/megatree-inference/supersmart/megatree-supersmart-sorted.dnd')
...
tab <- read.table('../results/2016-02-04-clusters/megatree-inference/supersmart/clade-species.tsv')
clades <- levels(tab[,1])
...
pdf("../results/plots/fig5b.pdf", 20, 20)
...
## for figure S5
pdf("../results/plots/figS5.pdf", 30, 50)
mycophyloplot(rep, final, space=100, gap=5, assoc=association, length.line=10, lwd=1, col=colors, exemplars=exemplars, show.tip.label=T, use.edge.length=T)
dev.off();
